...
export default class Render{
    ...
    getLayerContext(layer){
        if(!isIndexValid(layer, this.layers)){
            throw errorIndexOutOfRange(layer, this.layers);
        }
...
function errorIndexOutOfRange(index, list){
    throw Error(`Index ${index} out of range! The last valid index is ${list.length - 1}`);
}
